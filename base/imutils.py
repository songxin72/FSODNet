...
# from scipy.misc import imread, imsave, imresize  # 源代码
...
def get_image(path, height=256, width=256, flag=False):
    if flag is True:
        image = imread(path)  # imread(path, mode='RGB')
    else:
        image = imread(path, as_gray=True)  # imread(path, mode='L')  # Mode L is reading grayscale map.

    if height is not None and width is not None:  # 源代码进行格式统一
       image = np.array(Image.fromarray(image).resize((height, width)))  # imresize(image, [height, width], interp='nearest')  # 源代码
    return image


def get_gray_image(path, height=256, width=256, flag=False):
    padding = 0
    input_ir = imread(path) / 127.5 - 1.0
    input_ir = np.lib.pad(input_ir, ((padding, padding), (padding, padding)), 'edge')
    w, h = input_ir.shape
    image = input_ir.reshape([1, w, h])
    return image
...
def get_test_image(paths, height=None, width=None):
    ...
    images = []
    for path in paths:
        image = imread(path, as_gray=True)  # imread(path, mode='L')
