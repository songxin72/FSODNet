...
def load_framework(net_name):
    ...
    encoder, model, dlm_model = importlib.import_module('base.model').Network(net_name, config)
    ...
    if optim == 'SGD':
        if 'params' in config.keys():
            ...
            module_dlm = [{'params': getattr(dlm_model, p[0]).parameters(), 'lr': p[1]} for p in config['params']]
            optimizer_dlm = SGD(params=module_dlm, lr=config['lr'], momentum=0.9, weight_decay=0.0005, nesterov=True)
        else:
    ...
    return config, model, dlm_model, optimizer, optimizer_dlm,  scheduler, loss, saver
