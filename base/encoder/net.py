...
from .fusion_strategy import IMV_F
...
class SOSMaskFuse_autoencoder(nn.Module):
    ...
    def fusion(self, en1, en2):
        # attention weight 创建注意力函数
        fusion_function = IMV_F().Fusion
...
class SOSMaskFuse(nn.Module):
    def __init__(self, nb_filter, input_nc=1, output_nc=1):
        super(SOSMaskFuse, self).__init__()
        block = DenseBlock_light
        output_filter = 16
        kernel_size = 3
        stride = 1
