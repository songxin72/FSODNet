...
class Decoder_cyc(nn.Module):
    def __init__(self):
        super(Decoder_cyc, self).__init__()
        self.conv3 = reflect_conv(in_channels=128, kernel_size=3, out_channels=64, stride=1, pad=1)
        self.conv4 = reflect_conv(in_channels=64, kernel_size=3, out_channels=32, stride=1, pad=1)
        self.conv5 = nn.Conv2d(in_channels=32, kernel_size=1, out_channels=1, stride=1, padding=0)
...
class CyFusion(nn.Module):
    def __init__(self):
        ...
        self.encoder = Encoder()
        self.decoder = Decoder()
        self.decoder_1 = Decoder_cyc()
        self.decoder_2 = Decoder_cyc()
        self.fc = nn.Linear(128, 20)

    def forward(self, y_vi_image, ir_image):
        vi_encoder_out, ir_encoder_out, xs, xi = self.encoder(y_vi_image, ir_image)
        encoder_out = Fusion(vi_encoder_out, ir_encoder_out)
        fused_image = self.decoder(encoder_out)
        vi_out = self.decoder_1(ir_encoder_out)
        ir_out = self.decoder_2(vi_encoder_out)
        return fused_image, vi_out, ir_out, xs, xi


class TyFusion(nn.Module):

    def __init__(self):
        super(TyFusion, self).__init__()
        self.encoder = Encoder()
        self.decoder = Decoder()
        self.decoder_1 = Decoder_cyc()
        self.decoder_2 = Decoder_cyc()
