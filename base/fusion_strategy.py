...
class IMV_F(torch.nn.Module):
    ...
    def Salient_Background_Fusion(self, en_ir_a, en_ir_b, en_vi_a, en_vi_b):  # Salient:en_ir_a and en_vi_a.
        ...
        # SCA_5 = fusion_function(en_ir_b[4], en_vi_b[4])
        ...
        # SCA_weight_5 = SCA_5 / (SCA_5 + en_vi_b[4] + 1e-5)
        ...
        # f5_0 = (s_miu * en_ir_a[4] + (1 - s_miu) * en_vi_a[4]
        #         ) + 0.6 * (SCA_weight_5 * SCA_5 + (1 - SCA_weight_5) * en_vi_b[4]) + 0.4 * en_vi_b[4]
        return f1_0, f2_0, f3_0, f4_0

    def Salient_Background_Fusion2(self, en_ir_a, en_ir_b, en_vi_a, en_vi_b):  # Salient:en_ir_a and en_vi_a.
        SCA = Spatial_Channel_Attention()
        fusion_function = SCA.attention_fusion_weight

        SCA_1 = fusion_function(en_ir_b, en_vi_b)

        SCA_weight_1 = SCA_1 / (SCA_1 + en_vi_b + 1e-5)

        # The parameter μ=0.8, the optimal value obtained by the ablation experiment of Eq.(9) in the paper.
        s_miu = 0.8

        # fx_0 = (addition of significance) + 0.6 * (softmax for vi_b and SCA) + 0.4 * vi_b
        f1_0 = (s_miu * en_ir_a + (1 - s_miu) * en_vi_a
                ) + 0.6 * (SCA_weight_1 * SCA_1 + (1 - SCA_weight_1) * en_vi_b) + 0.4 * en_vi_b
