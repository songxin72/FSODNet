...
import glob
import os
import myutils.DatasetSplit
import myutils.transform as T
from myutils.randaugment import RandomAugment as RandomAugment
...
def custResize(img, new_shape=(224, 224), color=(0, 0, 0), scaleup=True):
    if(len(np.asarray(img).shape) == 2):
        img = cv2.cvtColor(np.asarray(img), cv2.COLOR_GRAY2BGR)
    else:
        img = cv2.cvtColor(np.asarray(img),cv2.COLOR_RGB2BGR)
