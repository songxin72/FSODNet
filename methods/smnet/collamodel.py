...
class decoder(nn.Module):
    ...
    def forward(self, xs, h, x_size):
        xs[4] = h[0].unsqueeze(0)
        xs[0] = self.adapter0(xs[0])  # [1, 16, 290, 460]
        xs[1] = self.adapter1(xs[1])  # [1, 16, 290, 460]
        xs[2] = self.adapter2(xs[2])  # [1, 16, 290, 460]
        xs[3] = self.adapter3(xs[3])  # [1, 16, 290, 460]
        xs[4] = self.adapter4(xs[4])  # [1, 16, 290, 460]

        glob_x = xs[4]  # [1, 16, 290, 460]
        reg_x, r3, r4 = self.region(xs[2:4], glob_x)  # reg_x: [1, 16, 290, 460]

        glob_x = self.gb_conv(glob_x)  # [1, 16, 1160, 1840]
        loc_x, r1, r2 = self.local(xs[0:2], glob_x)  # loc_x: [1, 16, 290, 460]

        reg_x = nn.functional.interpolate(reg_x, size=xs[0].size()[2:], mode=mode)  # [1, 16, 290, 460]
        pred = torch.sum(loc_x * reg_x, dim=1, keepdim=True)
        pred = nn.functional.interpolate(pred, size=x_size, mode='bilinear')  # [1, 1, 290, 460]
...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, num_slots=None):  # [1, 161098, 128]
        # inputs has shape [batch_size, num_inputs, inputs_size].
        inputs = self.norm_input(inputs)  # Apply layer norm to the input. [1, 161098, 128]
        k = self.project_k(inputs)  # Shape: [batch_size, num_inputs, slot_size]. [1, 161098, 128]
        v = self.project_v(inputs)  # Shape: [batch_size, num_inputs, slot_size]. [1, 161098, 128]
        # Initialize the slots. Shape: [batch_size, num_slots, slot_size].
        b, n, d = inputs.shape
        n_s = num_slots if num_slots is not None else self.num_slots
        slots = self.slots_embedding(torch.arange(0, n_s).expand(b, n_s).to(self.device))  # [1, 2, 128]
        # Multiple rounds of attention.
        for _ in range(self.iters):
            slots_prev = slots
            slots = self.norm_slots(slots)  # [1, 2, 128]
            # Attention.
            q = self.project_q(slots)  # Shape: [batch_size, num_slots, slot_size]. [1, 2, 128]
            dots = torch.einsum('bid,bjd->bij', q, k) * self.scale  # [1, 2, 161098]
            attn = dots.softmax(dim=1) + self.eps  # [1, 2, 161098]
            attn = attn / attn.sum(dim=-1, keepdim=True)  # weighted mean. [1, 2, 161098]
            updates = torch.einsum('bjd,bij->bid', v, attn)  # [1, 2, 128]
            # `updates` has shape: [batch_size, num_slots, slot_size].
            # Slot update.
            slots = self.gru(  # [2, 128]
                updates.reshape(-1, d),
                slots_prev.reshape(-1, d)
            )

            slots = slots.reshape(b, -1, d)  # [1， 2, 128]
            slots = slots + self.mlp(self.norm_pre_ff(slots))  # [1, 2, 128]
...
class Network(nn.Module):
    ...
    def forward(self, x, x1, phase='test'):
        x_size = x.size()[2:]
        # xs[0]:[1, 16, 290, 460] xs[1]:[1, 16, 290, 460] xs[3]:[1, 32, 290, 460] xs[4]:[1, 64, 290, 460]) xs[5]:[1, 128, 290, 460]
        fused_image, vis_rec, inf_rec, xs, xi = self.encoder(x, x1)  # 输出的格式：[batch, 128, 576, 768]

        # 可见光图像处理
        z = einops.rearrange(xs[4], 'b c h w -> b h w c')
        encoder_pos2 = SoftPositionEmbed(self.encoder_dims, (xs[4].shape[2], xs[4].shape[3]))
        z = encoder_pos2(z)  # Position embedding.  格式：[b, h, w, c] [1, 290, 460, 128]
        z = spatial_flatten(z)  # Flatten spatial dimensions (treat image as set).
        z = self.mlp2(self.layer_norm2(z))  # Feedforward network on set. 得到特征图 格式[batch, f1_dim, f2_dim] [1, 133400, 128]
        # Slot Attention module.
        slots = self.slot_attention2(z)  # [1, 2, 128]
        # `slots` has shape: [batch_size, num_slots, slot_size].

        # Spatial broadcast decoder.
        xz = spatial_broadcast(slots, (xs[4].shape[2], xs[4].shape[3]))  # 格式[2, 442368, 128, 128] [2, 290, 460, 128]
        # `x` has shape: [batch_size*num_slots, height_init, width_init, slot_size].
        decoder_pos2 = SoftPositionEmbed(self.encoder_dims, (xs[4].shape[2], xs[4].shape[3]))
        xz = decoder_pos2(xz)  # 将slot输入到编码器中 [2, 290, 460, 128]
        h = einops.rearrange(xz, 'b_n h w c -> b_n c h w')  # [2, 128, 290, 460]
        v_out, v_pos, v_neg = self.decoder(xs, h, x_size)

        # 红外图像处理
        z = einops.rearrange(xi[4], 'b c h w -> b h w c')
        encoder_pos = SoftPositionEmbed(self.encoder_dims, (xi[4].shape[2], xi[4].shape[3]))
        ...
        # `x` has shape: [batch_size*num_slots, height_init, width_init, slot_size].
        decoder_pos = SoftPositionEmbed(self.encoder_dims, (xi[4].shape[2], xi[4].shape[3]))
        xz = decoder_pos(xz)  # 将slot输入到编码器中
        h = einops.rearrange(xz, 'b_n h w c -> b_n c h w')
        i_out, i_pos, i_neg = self.decoder(xi, h, x_size)

        return v_out, i_out, v_pos, v_neg, i_pos, i_neg

    def encode_eval(self, input):
        encoder_out = self.encoder.encoder(input)
        return encoder_out

    def decode_eval(self, encoder_out):
        rec = self.encoder.decoder_eval(encoder_out)
        return rec
