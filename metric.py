...
class MetricRecorder:
    def __init__(self, eval_length_for_fm, beta_for_wfm=1):
        self.mae = cal_mae()  # mean absolute error
        self.fm = cal_fm(eval_length_for_fm)  # Fmeasure(maxFm, meanFm)
        self.sm = cal_sm()  # # Structure-measure
        self.em = cal_em()  # Enhanced-alignment Measure
...
class cal_fm(object):
    ...
    def show(self):
        ...
        fmeasure = 1.3 * precision * recall / (0.3 * precision + recall + 1e-8)  # 对应论文公式15
...
class cal_sm(object):
    ...
    def region(self, pred, gt):
        reg = center_of_mass(gt)  # 源代码:[y, x] = center_of_mass(gt)
        y = reg[0]
        x = reg[1]
        y = int(round(y)) + 1
        x = int(round(x)) + 1
